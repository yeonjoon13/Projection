...
import redHeart from '/public/assets/redHeart.jpg';
import yellowHeart from '/public/assets/yellow.webp';
import greenHeart from '/public/assets/health-care.png';
...
const ReportPage: React.FC<ReportPageProps> = ({ params }) => {
  ...
  const optimalRef = useRef<HTMLDivElement>(null);
  ...
  const ranges: Ranges = {
    ...
    BMI: { low: 0, belowOptimal: 16, optimal: 18.6, aboveOptimal: 25, high: 30 }
  };
    ...
    const fetchFileData = async () => {
      if (userId && fileName) {
        try {
          ...
          const response = await axios.get<string>(`/api/spring/api/v1/file-upload/fileData/${userId}/${fileName}`);
          const data = response.data;
          setFileData(data);
  
          const predictionResponse = await axios.post<{ interpretation: [string, string] }>(
            '/api/flask/process-file-data',
            data,
            {
              headers: {
  ...
  const parseData = (data: string): ParsedData => {
    ...
    const parsed = lines.reduce<ParsedData>((acc, line) => {
  ...
  const isAbnormal = (value: string, range?: Range): boolean => {
    if (!range) return false;
    const val = parseFloat(value);
    return val >= range.high;
  };

  const isOutsideOptimal = (value: string, range?: Range): boolean => {
    if (!range) return false;
    const val = parseFloat(value);
  ...
  return (
    <div>
      <Header collectedBy={collectedBy} collectedDate={collectedDate} />
