...
const FileUpload: React.FC<FileUploadProps> = ({ userId, userName }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post("/api/flask/upload", formData, {
        headers: {
      ...
      const s3Link = `https://${S3_BUCKET}.s3.${REGION}.amazonaws.com/uploads/${userId}/${selectedFile.name}`;
  
      await axios.post("/api/spring/api/v1/file-upload", {
        userId,
        userName,
